...
import { maxIntervalAtom } from "@/atoms/fieldAtoms";
import { ClockFadingIcon } from "lucide-react";

interface MaxIntervalInputProps {}
export const MaxIntervalInput = ({}: MaxIntervalInputProps) => {
  const [interval, setInterval] = useAtom(maxIntervalAtom);
  ...
  return (
    <div className="relative group">
      ...
      <div className="absolute top-4 right-4 z-10 bg-muted/80 border border-border/50 rounded-full py-2 px-3 flex items-center gap-2 backdrop-blur-sm">
        <ClockFadingIcon className="w-4 h-4 text-primary" />
      </div>
